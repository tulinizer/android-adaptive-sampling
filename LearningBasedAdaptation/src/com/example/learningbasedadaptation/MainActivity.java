package com.example.learningbasedadaptation;
...
public class MainActivity extends Activity implements SensorEventListener {
	...
	public double probability = 0.99;
	public double alpha_inc = 0.9;
	public double alpha_dec = 0.1;
	/*
...
//
		omega.setText("probability: " + probability);
//
	...
	public void schedule() {
		...
		listenSensors = new Runnable() {
			public void run() {
				...
				if (move > notMove && probability < 0.99 ) {
					probability = probability + alpha_inc * (1-probability);
					PERIOD = (int) Math.rint((SENSE/1000 / probability) + SENSE/1000);
					System.out.println("prob: " +probability + "period: " + PERIOD);
					scheduler.shutdownNow();
					setupTimer(PERIOD);
				}

				/* not interesting event increase waiting period */
				else if (move < notMove && probability > 0.1) {
					probability = probability - (alpha_dec * probability);
					PERIOD = (int) Math.rint((SENSE/1000 / probability) + SENSE/1000);
					System.out.println("prob: " +probability + "period: " + PERIOD);
					scheduler.shutdownNow();
					setupTimer(PERIOD);
