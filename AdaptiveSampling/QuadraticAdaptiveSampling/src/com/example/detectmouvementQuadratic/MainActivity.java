package com.example.detectmouvementQuadratic;
...
public class MainActivity extends Activity implements SensorEventListener {
	...
	public void onSensorChanged(SensorEvent event) {
		...
		moveNotMove.setText("Did I Move? (Yes, if number is positive): "
				+ (move - notMove));
	...
	public void schedule() {
		...
		listenSensors = new Runnable() {
			public void run() {
				...
				if (move > notMove && PERIOD > 32) {
					PERIOD = (int) (Math.sqrt(PERIOD - 30) + 30);
				...
				else if (move < notMove && PERIOD < 286) {
					PERIOD = (PERIOD - 30) * (PERIOD - 30) + 30;
					System.out.println("period: " + PERIOD);
					scheduler.shutdownNow();
					setupTimer(PERIOD);
				}
	...
	public void setupTimer(long duration) {
		...
		System.out.println("delay: " + (PERIOD - 30));
		
		sensorPeriodHandle = scheduler.scheduleAtFixedRate(listenSensors,
				(PERIOD - 30), duration, SECONDS);
	}
